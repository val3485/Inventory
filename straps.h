...
struct StrapItems 
{
	...
	StrapItems(int i, string ct, string c, string b, string lt, int q, string s, double p, string h)
			: isActive(true), id(i), categ(ct), color(c), brand(b), leather_type(lt), quantity(q), size(s), kid_size(""), price(p), hole(h) {}
...
inline vector<StrapItems> allstraps_arr = {
